...
public class RenderRadBarProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RenderRadBarProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				RenderRadBarProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	public static void renderTexture(float x, float y, float depth, float angle, float scale, int color, int alignment) {
		...
		int width = (int)(RadioactiveModVariables.loaded__bar_width * GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0, GL11.GL_TEXTURE_WIDTH));
		int height = (int)(RadioactiveModVariables.loaded__bar_height * GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0, GL11.GL_TEXTURE_HEIGHT));
		int red = color >> 16 & 255;
		int green = color >> 8 & 255;
		int blue = color & 255;
		int alpha = color >>> 24;
		PoseStack poseStack = guiGraphics.pose();
		poseStack.pushPose();
		poseStack.translate(x, Minecraft.getInstance().getWindow().getGuiScaledHeight() - y, -depth);
		poseStack.mulPose(com.mojang.math.Axis.ZN.rotationDegrees(angle));
		...
		bufferBuilder.vertex(matrix4f, 0, -1, 0.0F).uv(0.0F, 0.0F).color(red, green, blue, alpha).endVertex();
		bufferBuilder.vertex(matrix4f, 0, 0, 0.0F).uv(0.0F, 1.0F).color(red, green, blue, alpha).endVertex();
		bufferBuilder.vertex(matrix4f, scale, 0, 0.0F).uv(scale, 1.0F).color(red, green, blue, alpha).endVertex();
		bufferBuilder.vertex(matrix4f, scale, -1, 0.0F).uv(scale, 0.0F).color(red, green, blue, alpha).endVertex();
	...
	private static void execute(@Nullable Event event) {
		if(RadioactiveModVariables.client_show_bar){
			double factor = Math.min((RadioactiveModVariables.client_radiation / 1000), 1);
			double radcol = Lerp3ColoursProcedure.execute(255 << 24 | 0 << 16 | 255 << 8 | 0, 255 << 24 | 255 << 16 | 255 << 8 | 0, 255 << 24 | 255 << 16 | 0 << 8 | 0, factor);
			if (target(2)) {
				if(RadioactiveModVariables.loaded__radbar_bg){
					RenderSystem.setShaderTexture(0, new ResourceLocation(RadioactiveModVariables.loaded__radbar_bg_texture));
					renderTexture((float) RadioactiveModVariables.loaded__bar_x, (float) RadioactiveModVariables.loaded__bar_y, -9, 0, 1, 0xffffffff, 4);
				}
				RenderSystem.setShaderTexture(0, new ResourceLocation(RadioactiveModVariables.loaded__radbar_texture));
				renderTexture((float) RadioactiveModVariables.loaded__bar_x, (float) RadioactiveModVariables.loaded__bar_y, -9, 0, (float) factor, (int) radcol, 4);
				release();
			}
		}
	}
