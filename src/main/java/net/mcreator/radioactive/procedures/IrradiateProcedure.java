		...
		if (RadioactiveCFGConfiguration.OLD_RADIATION.get() || RadioactiveCFGConfiguration.ONLY_PLAYER_RADIATION.get()) {
			{
				double _setval = Math.min(2147483647,
						Math.max((entity.getCapability(RadioactiveModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RadioactiveModVariables.PlayerVariables())).received_radiation
								+ amount * (double) RadioactiveCFGConfiguration.RADIATION_MULTIPLIER.get()
			...
			entity.getPersistentData().putDouble("radiation",
					(Math.min(2147483647, Math.max(entity.getPersistentData().getDouble("radiation") + amount * (double) RadioactiveCFGConfiguration.RADIATION_MULTIPLIER.get() * (1 - entity.getPersistentData().getDouble("rad_resistance")), 0))));
		}
	}
}
